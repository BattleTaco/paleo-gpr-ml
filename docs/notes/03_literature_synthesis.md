# Literature Synthesis: What the Papers Tell Me
...
There are three active research threads that my project sits at the intersection of:
...
**The gap**: Nobody has applied ML to GPR data for fossil detection. Threads 1 and 2 are parallel lines that don't intersect. My project connects them.
...
2. **How realistic gprMax fossil simulations are.** I can simulate fossil-like targets with the right dielectric values, but real geology is messy: heterogeneous soil, surface roughness, clay layers, water table effects. The simulation-to-reality gap is uncertain.

3. **Whether detection or segmentation is better for GPR anomalies.** Detection (bounding boxes) is simpler and matches my existing annotations. Segmentation could be more precise but requires pixel-level labels I don't have.

4. **The effect of GPR frequency on ML detection.** My dataset is from some unknown frequency. The paleontological papers use 400 MHz and 2 GHz. A model trained on one frequency may not work on another.

## The novel contribution

My project's novelty is clear and defensible:

**First ML-based approach to fossil detection in GPR data.** I'm not claiming a new architecture or a new training technique. I'm applying existing methods (YOLO/R-CNN, synthetic data, transfer learning) to a new problem (fossil prospecting from GPR) where no prior ML work exists.
...
These are answerable with the tools I have. The answers have practical value for paleontological fieldwork.

## Methodological lessons from the literature

**For my detection experiment:**
...
**For my synthetic data pipeline:**
